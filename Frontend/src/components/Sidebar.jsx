...
import { Trash2, ChevronDown, ChevronRight, FileText, Clock, Library, X } from "lucide-react"
import toast from "react-hot-toast"

export default function Sidebar({ currentSession, onSelectSession, isOpen = false, onClose = () => {} }) {
  ...
  return (
    <>
      {/* FIX: dark backdrop behind the drawer on mobile, tap to close */}
      {isOpen && (
        <div
          onClick={onClose}
          className="fixed inset-0 top-16 bg-black/60 z-30 sm:hidden"
        />
      )}

      {/* FIX: fixed slide-in drawer on mobile (<sm), normal static panel on sm+ */}
      <aside
        className={`fixed sm:static top-16 sm:top-0 left-0 bottom-0 sm:bottom-auto z-40 w-72 max-w-[85vw] sm:max-w-none
          glass border-r border-white/5 flex flex-col h-[calc(100vh-4rem)] sm:h-full overflow-hidden
        ...
        <div className="px-5 py-4 border-b border-border/70 flex items-center justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <Library className="w-4 h-4 text-primary-soft flex-shrink-0" />
              <h2 className="text-text font-semibold text-sm tracking-tight truncate">Session History</h2>
            </div>
          ...
          <button
            ...
            className="sm:hidden p-1.5 rounded-lg text-muted hover:text-primary-soft hover:bg-white/5 flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
